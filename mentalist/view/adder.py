...
class AdderNode(BaseWordsNode):
    ...
    def open_special_dlg(self):
        ...
        for v, val in enumerate(SPECIAL_CHARACTERS):
            ...
            #print(v,val,var)
            if val == "A":
                tmp = Tk.Checkbutton(box, text="All", relief=Tk.FLAT, variable=var)
            else:
        ...
        #tmp = Tk.Checkbutton(box,text="all",relief=Tk.FLAT,variable=var)
        #self.chk_special.append(var)
        #tmp.grid(row=v+1 % max_column_checks, column=v // max_column_checks,
        #             sticky='W', padx=10)
    ...
    def on_ok_special_dlg(self, *args):
        ...
        '''
        all_special_selected = False
        checked_vals = []
        for i in range(len(SPECIAL_CHARACTERS)):
            if self.chk_special[i].get() == 1:
                checked_vals.append(SPECIAL_CHARACTERS[i])
                if SPECIAL_CHARACTERS[i] == "A":
                    all_special_selected = True

        if all_special_selected == True:
            checked_vals = SPECIAL_CHARACTERS[:-1]
